...
abstract contract PrecompileConsumer {
    ...
    function _executePrecompile(
        ...
        // ── Gas forwarding ──────────────────────────────────────────────────
        // We must forward gas explicitly. Without it, the EVM 63/64 rule
        // and Solidity overhead can leave the async LLM precompile (0x0802)
        // with far too little gas to complete the commitment handshake.
        // Reserve 5 000 gas for post-call cleanup (emit, storage writes).
        uint256 forwardGas = gasleft() - 5_000;
        bool success;
        bytes memory rawOutput;
        assembly {
            // allocate return buffer
            rawOutput := mload(0x40)
            let outLen := returndatasize()
            // call with explicit gas
            success := call(forwardGas, precompile, 0, add(input, 32), mload(input), 0, 0)
            outLen := returndatasize()
            // copy returndata into rawOutput
            mstore(0x40, add(rawOutput, add(outLen, 0x20)))
            mstore(rawOutput, outLen)
            returndatacopy(add(rawOutput, 0x20), 0, outLen)
        }
